...
For a 50-page methodology document, I would not put the entire document into the prompt by default. I would first separate stable global context from task-specific context. Stable context includes the agent role, answer policy, output format, schema definitions, map-layer definitions, and terminology that rarely changes. Dynamic context includes the user question, selected web map, active layer, time range, and retrieved methodology excerpts. I would place the stable/static content first in the prompt and append dynamic content afterward so repeated calls can benefit from prefix/KV-cache reuse where the model provider supports it. The model should not have to infer which parts of a long methodology are relevant from a full-document dump every time.
